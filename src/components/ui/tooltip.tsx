...
>(({ className, sideOffset = 0, children, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(tooltipContentBase, className)}
      {...props}
    >
      {children}
      <TooltipPrimitive.Arrow className="z-50 size-2.5 translate-y-[calc(-50%-2px)] rotate-45 rounded-xs fill-primary bg-primary" />
    </TooltipPrimitive.Content>
  </TooltipPrimitive.Portal>
))
